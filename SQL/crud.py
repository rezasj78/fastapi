...
def get_repairman_by_phone(db: Session, repairman_phone: int):
	return db.query(models.Repairman).filter(models.Repairman.phone_num == repairman_phone).first()
...
def create_repairman(db: Session, repairman: schemas.RepairmanCreate):
	...
	db_repairman = models.User(name=repairman.name, phone_num=repairman.phone_num,
